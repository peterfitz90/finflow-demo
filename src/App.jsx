...
function Invoices({ companyName, companyId: propCid, company }) {
  ...
  const finaliseDoc = async () => {
    ...
    try {
      await finaliseInvoice(supabase, cid, formInv, formLines, customers, invSettings);
      await loadData();
      setModal(null);
  ...
  const saveDraft = async () => {
    ...
    setSaving(true); setSaveErr(null);
    try {
      const invId = await createInvoiceDraft(supabase, cid, formInv, formLines, customers, invSettings, baseCurrency);
      if (!formInv.id) setFormInv(p => ({ ...p, id: invId }));
      await loadData();
      setModal(null);
    } catch (err) { setSaveErr(err.message); }
    setSaving(false);
